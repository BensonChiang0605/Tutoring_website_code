...
def correct_grammar(scanned_text, essay_question):
    openai.api_key = "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"
...
def submit_essay():
    ...
    if form.validate_on_submit():
        ...
        # correct grammar of raw_essay create variable called essay
            post = Post(title=f"{current_user.username} response to '{form.prompt.data}'", content=correct_grammar(scanned_text,form.prompt.data), author=current_user)
            db.session.add(post)
            db.session.commit()
            flash('Post created!', 'success')
        return redirect(url_for('home'))
